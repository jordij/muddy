...
import itertools
from constants import (TIMEZONE, AQUADOPPS_HEIGHTS, ADCP_DATES, AQUADOPPS_SENS,
                       AQUADOPPS_LEVELS, AQUADOPPS_A1_LEVELS,
                       SIGNATURE1000_HEIGHTS)
...
FILEPATH = "C:/Users/jordi/code/muddy/data/Currents/ADCP/"
...
class Signature1000(object):
    ...
    def __init__(self, site):
        ...
        self.wd = wd
        # Main dataframe
        index = list(itertools.product(timestamps, SIGNATURE1000_HEIGHTS))
        ...
        # clean data
        if AQUADOPPS_A1_LEVELS[self.site - 1] is not None:
            vars_nan = self.VARS[-(len(self.VARS)-1):] + self.AMPLITUDES
            self.df.loc[self.df.a1 < AQUADOPPS_A1_LEVELS[self.site - 1],
                        vars_nan] = np.NaN
    ...
    def clean_by_hah(self, threshold=1, min_hah=1):
        # agressive cleanup
        vars_nan = self.VARS[-(len(self.VARS)-1):] + self.AMPLITUDES
        lower_idx = list(filter(lambda x: x > min_hah,
                                SIGNATURE1000_HEIGHTS))
        self.df["OutOfWater"] = self.df.apply(
            lambda r: (r.WaterDepth - r.name[1]) < threshold,  # depth - hah
            axis=1)
        r_df = self.df.reset_index()
        # idx = pd.IndexSlice
        r_df.loc[((r_df.Height > min_hah) & (r_df.OutOfWater)), vars_nan] = np.NaN
...
class Aquadopp(object):
    ...
    def __init__(self, site):
        ...
        if AQUADOPPS_A1_LEVELS[self.site - 1] is not None:
            self.df.loc[self.df.a1 < AQUADOPPS_A1_LEVELSS[self.site - 1],
                        self.VARS] = np.NaN
